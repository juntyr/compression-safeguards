...
- [`xarray-safeguards`][xarray_safeguards]: provides functionality to use safeguards with (chunked) [`xarray.DataArray`][xarray.DataArray]s.
...
We recommend using the safeguards through one of their integrations with popular compression APIs, e.g. [`numcodecs-safeguards`][numcodecs_safeguards] for non-chunked data:
