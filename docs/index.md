...
- [`numcodecs-safeguards`][numcodecs_safeguards]: provides the [`SafeguardedCodec`][numcodecs_safeguards.SafeguardedCodec] meta-compressor that conveniently applies safeguards to any compressor using the [`numcodecs.abc.Codec`][numcodecs.abc.Codec] API.
...
- [`same`][compression_safeguards.safeguards.pointwise.same.SameValueSafeguard] (exact value preserving):

    If an element has a special value in the input, that element is guaranteed to also have bitwise the same value in the decompressed output. This safeguard can be used for preserving e.g. zero values, missing values, pre-computed extreme values, or any other value of importance. By default, elements that do *not* have the special value in the input may still have the value in the output. It is also possible to enforce that an element in the output only has the special value if and only if it also has the value in the input, e.g. to ensure that only values that are missing in the input have the missing value sentinel bitpattern in the output. Beware that +0.0 and -0.0 are semantically equivalent in floating-point but have different bitwise patterns. To preserve both, two same value safeguards are needed, one for each bitpattern. Alternatively, to preserve both interchangibly, use one [`equivalent`][compression_safeguards.safeguards.pointwise.same.EquivalentValueSafeguard] value safeguard instead. Further beware that there are many NaN bit patterns. To preserve all of them, use one [`equivalent`][compression_safeguards.safeguards.pointwise.same.EquivalentValueSafeguard] value safeguard instead.

- [`equivalent`][compression_safeguards.safeguards.pointwise.same.EquivalentValueSafeguard] (equivalent value preserving):

    If an element has a special value in the input, that element is guaranteed to also have an equivalent value in the decompressed output. This safeguard can be used for preserving e.g. zero values, NaN values, missing values, pre-computed extreme values, or any other value of importance. By default, elements that are *not* equivalent to the special value in the input may still have an equivalent value in the output. It is also possible to enforce that an element in the output is only equivalent to the special value if and only if it is also equivalent to the value in the input, e.g. to ensure that only values that are missing in the input are equivalent to the missing value sentinel in the output. This safeguard preserves zero values as positive or negative zero interchangibly. To preserve their exact bit patterns, use one or two [`same`][compression_safeguards.safeguards.pointwise.same.SameValueSafeguard] value safeguards instead. This safeguard preserves NaN values as NaN values with any payload. To preserve a specific bit pattern, use a [`same`][compression_safeguards.safeguards.pointwise.same.SameValueSafeguard] value safeguard instead.
...
You can get started quickly with the [`numcodecs`][numcodecs]-compatible [`SafeguardedCodec`][numcodecs_safeguards.SafeguardedCodec] meta-compressor for non-chunked arrays:
