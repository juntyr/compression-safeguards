"""
Safeguards for preserving special values.
"""

__all__ = ["SameValueSafeguard", "EquivalentValueSafeguard"]
...
class SameValueSafeguard(PointwiseSafeguard):
    ...
    def check_pointwise(
        ...
        *,
        ...
        """
        Check which elements bitwise preserve the special `value` from the
        `data` to the `approximation` array.
...
class EquivalentValueSafeguard(PointwiseSafeguard):
    """
    The `EquivalentValueSafeguard` guarantees that if an element has a special
    `value` in the input, that element also has an equivalent value in the
    decompressed output.

    This safeguard can be used for preserving e.g. zero values, NaN values,
    ...
    def check_pointwise(
        ...
        *,
        ...
        if self._exclusive:
            ...
            ok = np.where(
                np.isnan(value),
                np.isnan(data) == np.isnan(approximation),
                (data == value) == (approximation == value),
            )
        else:
            # value must stay value, everything else can be arbitrary
            ok = np.where(
    ...
    def compute_safe_intervals(
        ...
        *,
        ...
        """
        Compute the intervals in which the equivalent value guarantee is upheld
        ...
        -------
        intervals : IntervalUnion[T, int, int]
            Union of intervals in which the equivalent value guarantee is
            upheld.
        ...
        if not self._exclusive:
            ...
            if np.issubdtype(data.dtype, np.floating) and np.any(valuef == 0):
                ...
                Lower(np.array(neg_zero)) <= valid[
                    (dataf == 0) & (valuef == 0)
                ] <= Upper(np.array(pos_zero))
            # preserve NaN elements as NaN values with any bit pattern
            if np.issubdtype(data.dtype, np.floating) and np.any(np.isnan(valuef)):
                return valid.preserve_any_nan(
                    dataf, equal_nan=True, where=np.isnan(valuef)
        ...
        with np.errstate(over="ignore", under="ignore"):
            valuef_total_below = valuef_total - 1
            valuef_total_above = valuef_total + 1

        # preserve value elements equivalently
        ...
        if np.issubdtype(data.dtype, np.floating) and np.any(valuef == 0):
            ...
            with np.errstate(over="ignore", under="ignore"):
                valuef_total_below -= _is_positive_zero(valuef)  # type: ignore
                valuef_total_above += _is_negative_zero(valuef)  # type: ignore

        below_upper = _ensure_array(from_total_order(valuef_total_below, data.dtype))
        above_lower = _ensure_array(from_total_order(valuef_total_above, data.dtype))
