...
class ScalarSquare(Expr[Expr]):
    ...
    def compute_data_bounds_unchecked(
        ...
        np.negative(au, out=arg_lower, where=(np.less_equal(expr_lower, 0) | _is_sign_negative_number(argv)))
        arg_lower = _minimum_zero_sign_sensitive(argv, arg_lower)

        arg_upper: np.ndarray[Ps, np.dtype[F]] = np.array(au, copy=True)
        np.negative(al, out=arg_upper, where=(np.greater(expr_lower, 0) & _is_sign_negative_number(argv)))
